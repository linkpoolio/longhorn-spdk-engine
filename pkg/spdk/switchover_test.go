...
func (s *TestSuite) TestEngineFrontendSwitchOverTargetNvmfSuccess(c *C) {
	...
	ef := NewEngineFrontend("ef-a", "engine-a", "vol-a", lhtypes.FrontendSPDKTCPNvmf, 1024, 0, 0, NvmfTransportTCP, updateCh)
...
func (s *TestSuite) TestEngineFrontendSwitchOverTargetBlockdevRunningUsesMultipathConnect(c *C) {
	...
	updateCh := make(chan interface{}, 1)
	ef := NewEngineFrontend("ef-a", "engine-a", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 1024, 0, 0, NvmfTransportTCP, updateCh)
	ef.State = lhtypes.InstanceStateRunning
	ef.NvmeTcpFrontend.TargetIP = "10.0.0.1"
	ef.NvmeTcpFrontend.TargetPort = 2000
...
func (s *TestSuite) TestServerEngineFrontendSwitchOverAmbiguousEngineName(c *C) {
	...
	srv := &Server{
		engineFrontendMap: map[string]*EngineFrontend{
			"ef-a": NewEngineFrontend("ef-a", "engine-a", "vol-a", lhtypes.FrontendSPDKTCPNvmf, 1024, 0, 0, NvmfTransportTCP, make(chan interface{}, 1)),
			"ef-b": NewEngineFrontend("ef-b", "engine-a", "vol-a", lhtypes.FrontendSPDKTCPNvmf, 1024, 0, 0, NvmfTransportTCP, make(chan interface{}, 1)),
		},
	}
...
func (s *TestSuite) TestEngineFrontendSwitchOverTargetUsesPathMetadataForRemoteANASync(c *C) {
	...
	ef := NewEngineFrontend("ef-a", "", "vol-a", lhtypes.FrontendSPDKTCPNvmf, 1024, 0, 0, NvmfTransportTCP, make(chan interface{}, 1))
	ef.State = lhtypes.InstanceStateRunning
	ef.NvmeTcpFrontend.TargetIP = "10.0.0.1"
	ef.NvmeTcpFrontend.TargetPort = 2000
	ef.NvmeTcpFrontend.Nqn = getStableVolumeNQN("vol-a")

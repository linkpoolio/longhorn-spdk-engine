...
// during a live migration / engine upgrade never collide. See getEngineCntlidRange.
...
func (e *Engine) Delete(spdkClient *spdkclient.Client, superiorPortAllocator *commonbitmap.Bitmap) (err error) {
	...
	case types.FrontendSPDKTCPBlockdev, types.FrontendSPDKTCPNvmf:
		...
		// per-subsystem RPCs (nvmf_subsystem_get_listeners, nvmf_delete_subsystem)
		// hang and only return on the client timeout — e.g. after a failed
		// replica rebuild leaves the subsystem stuck. nvmf_get_subsystems still
		// responds instantly, so the target is healthy; only operations on this
		// one subsystem block, and no graceful SPDK call clears it short of an
		// instance-manager restart. Treating that as fatal here would pin the
		// volume in `deleting` forever while the initiator's reconnect loop
		// floods keep-alive timeouts. Proceed regardless — the orphaned
	...
	// leftover namespace and BdevRaidDelete can fail/hang. Don't let that pin the
	// engine in `deleting` — proceed and let the instance-manager restart reclaim it.
	...
	// Best-effort: a failure to detach the engine's replica initiator connections
	// must not block engine removal either. Replica lvol teardown happens
	// independently when the replica CRs are deleted.

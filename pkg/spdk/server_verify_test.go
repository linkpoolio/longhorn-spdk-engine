...
func (s *TestSuite) TestHandleVerifyErrorBrokenPipe(c *C) {
	...
	engine := NewEngine("e1", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 1024, NvmfTransportTCP, make(chan interface{}, 1), defaultTestSnapshotMaxCount)
...
func (s *TestSuite) TestHandleVerifyErrorBrokenPipeKeepsStoppedAndError(c *C) {
	...
	engineErrored := NewEngine("e-err", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 1024, NvmfTransportTCP, make(chan interface{}, 1), defaultTestSnapshotMaxCount)
...
func (s *TestSuite) TestNewVerifyStateLockedCopiesMaps(c *C) {
	...
	server := &Server{
		...
		engineMap: map[string]*Engine{
			"e1": NewEngine("e1", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 1024, NvmfTransportTCP, make(chan interface{}, 1), defaultTestSnapshotMaxCount),
		},
...
func (s *TestSuite) TestApplyVerifiedStateSkipsConcurrentReplicaMutation(c *C) {
	...
	r1 := NewReplica(context.Background(), "r1", "disk-a", "uuid-a", 1024, true, NvmfTransportTCP, make(chan interface{}, 1))
	r2 := NewReplica(context.Background(), "r2", "disk-a", "uuid-a", 1024, true, NvmfTransportTCP, make(chan interface{}, 1))
	replacement := NewReplica(context.Background(), "r2", "disk-a", "uuid-a", 1024, true, NvmfTransportTCP, make(chan interface{}, 1))

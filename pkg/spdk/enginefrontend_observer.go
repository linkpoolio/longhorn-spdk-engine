...
func ObserveEngineFrontend(ctx context.Context, spdkClient *spdkclient.Client, record *EngineFrontendRecord) (live *EngineFrontendLive, err error) {
	...
	// Diagnostic breadcrumb for the derived-state view. Logged at Debug so it
	// is silent at the default log level and can be turned on by raising the
	// data-engine log level. Without it, a blockdev whose layers all probe
	// absent derives Stopped (0b0000), which reconcileOnce treats as
	// non-Error and silently skips — a frontend that is physically up but
	// observed-down would leave no trace. The named return values let this
	// fire on every exit path, capturing the exact layer bitmap the observer
	// saw alongside the state it derived.
	defer func() {

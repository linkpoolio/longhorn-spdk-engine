...
func (s *TestSuite) TestEngineFrontendCreateReturnsErrorForFrontendFailure(c *C) {
	...
	ef := NewEngineFrontend("ef-test", "engine-a", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 1024, 0, 0, NvmfTransportTCP, make(chan interface{}, 1))
...
func (s *TestSuite) TestCreateDoesNotHoldLockWhileSendingUpdate(c *C) {
	...
	ef := NewEngineFrontend("ef-create-lock", "engine-a", "vol-a", lhtypes.FrontendEmpty, 1024, 0, 0, NvmfTransportTCP, make(chan interface{}))
...
func (s *TestSuite) TestConcurrentCreateHasSingleWinner(c *C) {
	...
	ef := NewEngineFrontend("ef-create-concurrent", "engine-a", "vol-a", lhtypes.FrontendEmpty, 1024, 0, 0, NvmfTransportTCP, make(chan interface{}, 32))

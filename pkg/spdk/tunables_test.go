...
// These cover the env-tunable + cntlid helpers that back the LONGHORN_V2_*
// override apparatus. They are pure / env-driven, so they run as standard
// go tests alongside the gocheck suite (Test in util_test.go).
...
func TestAccelMlx5NumRequests(t *testing.T) {
	// A 2-core cpumask with no explicit override yields cores * mkeys-per-core.
	t.Run("derived from core count", func(t *testing.T) {
		t.Setenv("LONGHORN_V2_SPDK_CPUMASK", "0x3")
		os.Unsetenv("LONGHORN_V2_ACCEL_MLX5_NUM_REQUESTS")
	...
	t.Run("explicit override wins", func(t *testing.T) {
		t.Setenv("LONGHORN_V2_SPDK_CPUMASK", "0x3")
		t.Setenv("LONGHORN_V2_ACCEL_MLX5_NUM_REQUESTS", "999")
		if got := accelMlx5NumRequests(); got != 999 {
			t.Fatalf("accelMlx5NumRequests() with override = %d, want 999", got)
		}
	})

	t.Run("non-positive override is ignored", func(t *testing.T) {
		t.Setenv("LONGHORN_V2_SPDK_CPUMASK", "0x3")
		t.Setenv("LONGHORN_V2_ACCEL_MLX5_NUM_REQUESTS", "0")
		want := uint32(2) * accelMlx5MkeysPerCore
		if got := accelMlx5NumRequests(); got != want {
			t.Fatalf("accelMlx5NumRequests() with 0 override = %d, want fallback %d", got, want)
		}
	})
}

...
func (s *TestSuite) TestReplicaHealExposeParams(c *C) {
	...
	rRDMA := NewReplica(context.Background(), "r-heal", "lvs", "lvs-uuid", 1024, false, NvmfTransportRDMA, make(chan interface{}, 1), nil)
	...
	rTCP := NewReplica(context.Background(), "r-heal", "lvs", "lvs-uuid", 1024, false, NvmfTransportTCP, make(chan interface{}, 1), nil)
...
func (s *TestSuite) TestReplicaRecordSurvivesCleanStop(c *C) {
	...
	metadataDir := c.MkDir()
	r := NewReplica(context.Background(), "r-stop", "lvs", "lvs-uuid", 1024, false, NvmfTransportTCP, make(chan interface{}, 1), nil)

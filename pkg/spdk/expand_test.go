...
func (s *TestSuite) TestEngineExpandPrecheckGuards(c *C) {
	...
	eInProgress := NewEngine("engine-a", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 10, NvmfTransportTCP, make(chan interface{}, 1), defaultTestSnapshotMaxCount, nil)
	eInProgress.isExpanding = true
	_, err := eInProgress.ExpandPrecheck(nil, 20)
	c.Assert(errors.Is(err, ErrExpansionInProgress), Equals, true)

	eRestoring := NewEngine("engine-b", "vol-b", lhtypes.FrontendSPDKTCPBlockdev, 10, NvmfTransportTCP, make(chan interface{}, 1), defaultTestSnapshotMaxCount, nil)
...
func (s *TestSuite) TestHandleReplicaExpandResult(c *C) {
	...
	eAllFailed := NewEngine("engine-a", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 10, NvmfTransportTCP, make(chan interface{}, 1), defaultTestSnapshotMaxCount, nil)
	...
	ePartial := NewEngine("engine-b", "vol-b", lhtypes.FrontendSPDKTCPBlockdev, 10, NvmfTransportTCP, make(chan interface{}, 1), defaultTestSnapshotMaxCount, nil)
...
func (s *TestSuite) TestEngineExpandPreservesANAState(c *C) {
	...
	e2 := NewEngine("engine-b", "vol-b", lhtypes.FrontendSPDKTCPBlockdev, 10, NvmfTransportTCP, make(chan interface{}, 1), defaultTestSnapshotMaxCount, nil)

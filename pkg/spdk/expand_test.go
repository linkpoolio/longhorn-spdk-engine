...
func (s *TestSuite) TestEngineFrontendFinishExpansionSuccessClearsErrorState(c *C) {
	...
	ef := NewEngineFrontend("ef-a", "engine-a", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 10, 0, 0, NvmfTransportTCP, make(chan interface{}, 1))
...
func (s *TestSuite) TestEngineFrontendRequireExpansionGuards(c *C) {
	...
	efInProgress := NewEngineFrontend("ef-a", "engine-a", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 10, 0, 0, NvmfTransportTCP, make(chan interface{}, 1))
	...
	efRestoring := NewEngineFrontend("ef-b", "engine-b", "vol-b", lhtypes.FrontendSPDKTCPBlockdev, 10, 0, 0, NvmfTransportTCP, make(chan interface{}, 1))
	...
	efSmaller := NewEngineFrontend("ef-c", "engine-c", "vol-c", lhtypes.FrontendSPDKTCPBlockdev, 20, 0, 0, NvmfTransportTCP, make(chan interface{}, 1))
	_, err = efSmaller.requireExpansion(context.Background(), nil, 10)
	c.Assert(errors.Is(err, ErrExpansionInvalidSize), Equals, true)

	efUnaligned := NewEngineFrontend("ef-d", "engine-d", "vol-d", lhtypes.FrontendSPDKTCPBlockdev, 10*helpertypes.MiB, 0, 0, NvmfTransportTCP, make(chan interface{}, 1))
...
func (s *TestSuite) TestEngineExpandPrecheckGuards(c *C) {
	...
	eInProgress := NewEngine("engine-a", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 10, NvmfTransportTCP, make(chan interface{}, 1))
	eInProgress.isExpanding = true
	_, err := eInProgress.ExpandPrecheck(nil, 20)
	c.Assert(errors.Is(err, ErrExpansionInProgress), Equals, true)

	eRestoring := NewEngine("engine-b", "vol-b", lhtypes.FrontendSPDKTCPBlockdev, 10, NvmfTransportTCP, make(chan interface{}, 1))
...
func (s *TestSuite) TestHandleReplicaExpandResult(c *C) {
	...
	eAllFailed := NewEngine("engine-a", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 10, NvmfTransportTCP, make(chan interface{}, 1))
	...
	ePartial := NewEngine("engine-b", "vol-b", lhtypes.FrontendSPDKTCPBlockdev, 10, NvmfTransportTCP, make(chan interface{}, 1))
...
func (s *TestSuite) TestExpandDoesNotBlockConcurrentGet(c *C) {
	...
	ef := NewEngineFrontend("ef-a", "engine-a", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 1024, 0, 0, NvmfTransportTCP, make(chan interface{}, 1))
...
func (s *TestSuite) TestExpandCapturesTargetAddressWithNilNvmeTcpFrontend(c *C) {
	...
	ef := NewEngineFrontend("ef-a", "engine-a", "vol-a", lhtypes.FrontendUBLK, 1024, 0, 0, NvmfTransportTCP, make(chan interface{}, 1))
...
func (s *TestSuite) TestEngineExpandPreservesANAState(c *C) {
	...
	e2 := NewEngine("engine-b", "vol-b", lhtypes.FrontendSPDKTCPBlockdev, 10, NvmfTransportTCP, make(chan interface{}, 1))
...
func (s *TestSuite) TestGetEngineServiceAddressFrontendEmpty(c *C) {
	...
	ef := NewEngineFrontend("ef-a", "engine-a", "vol-a", lhtypes.FrontendEmpty, 1024, 0, 0, NvmfTransportTCP, make(chan interface{}, 1))
...
func (s *TestSuite) TestCreateSetsEngineIPForEmptyFrontend(c *C) {
	...
	ef := NewEngineFrontend("ef-empty", "engine-a", "vol-a", lhtypes.FrontendEmpty, 1048576, 0, 0, NvmfTransportTCP, make(chan interface{}, 1))
...
func (s *TestSuite) TestCreateSetsEngineIPForBlockdevFrontend(c *C) {
	...
	ef := NewEngineFrontend("ef-block", "engine-a", "vol-a", lhtypes.FrontendSPDKTCPBlockdev, 1048576, 0, 0, NvmfTransportTCP, make(chan interface{}, 1))

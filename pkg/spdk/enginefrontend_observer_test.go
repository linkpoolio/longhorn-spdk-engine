...
// riskiest behavior lives precisely in this mapping (a physically-up blockdev
// observed with all layers absent derives Stopped, which the reconciler
// silently skips), so these cases pin every blockdev layer combination plus
// the nvmf / empty / unknown arms.
	...
	cases := []deriveLiveStateCase{
		{
			...
			// is also the exact shape a mis-observed but actually-up frontend
			// produces — Stopped, non-Error, silently skipped by the
			// reconciler. deriveLiveState is correct here; an observation bug
			// would be surfaced by the observer's Debug log.
...
func (s *TestSuite) TestEngineFrontendHealSkipsDuringDeletion(c *C) {
	...
	ef := NewEngineFrontend("ef-del", "engine-del", "vol-del", lhtypes.FrontendSPDKTCPBlockdev, 1024, 0, 0, make(chan interface{}, 1), nil)
	...
	ef2 := NewEngineFrontend("ef-del2", "engine-del2", "vol-del", lhtypes.FrontendSPDKTCPBlockdev, 1024, 0, 0, make(chan interface{}, 1), nil)

...
// current ValidateAndUpdate pass, captures the SPDK-side dirty bitmap so a
// subsequent replica-add can rebuild incrementally.
//
...
func (e *Engine) captureBitmapForReplicaNoLock(spdkClient *spdkclient.Client, replicaName, bdevName string) error {
	...
	if _, err := spdkClient.BdevRaidClearBaseBdevFaultyState(bdevName); err != nil {
		// The bitmap is already captured — failing to clear is not fatal.
...
// force it off by setting LONGHORN_V2_RAID_DELTA_BITMAP=0 on the
// instance-manager pod (e.g. if the base bdev layer exposes
// optimal_io_boundary=0 and would reject raid1 startup).

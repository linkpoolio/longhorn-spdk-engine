...
func (s *TestSuite) TestReplicaDeletePendingWithoutCleanupIsNoop(c *C) {
	...
	r := server.NewReplica(context.Background(), "test-replica-pending", "test-lvs", "test-lvs-uuid", 0, true, server.NvmfTransportTCP, make(chan interface{}, 1))
	c.Assert(r, NotNil)
...
func (s *TestSuite) TestSPDKEngineAndEngineFrontendCreateAndDeleteWithDifferentFrontends(c *C) {
	...
	for _, test := range tests {
		...
		func() {
			...
			engine, err := spdkCli.EngineCreate(engineName, volumeName, test.frontend, defaultTestLvolSize, replicaAddressMap, nil, 1, false, 0, nil)
			c.Assert(err, IsNil)
...
func (s *TestSuite) TestSPDKEngineFrontendSuspendAndResume(c *C) {
	...
	engine, err := spdkCli.EngineCreate(engineName, volumeName, types.FrontendSPDKTCPBlockdev, defaultTestLvolSize, replicaAddressMap, nil, 1, false, 0, nil)
	c.Assert(err, IsNil)
	c.Assert(engine.State, Equals, types.InstanceStateRunning)
	c.Assert(engine.ErrorMsg, Equals, "")
	c.Assert(engine.ReplicaAddressMap, DeepEquals, replicaAddressMap)
...
func (s *TestSuite) TestSPDKEngineFrontendSnapshotRevert(c *C) {
	...
	for _, tc := range tests {
		...
		func() {
			...
			engine, err := spdkCli.EngineCreate(engineName, volumeName, tc.engineFrontendType, defaultTestLvolSize, replicaAddressMap, nil, 1, false, 0, nil)
			c.Assert(err, IsNil)
...
func (s *TestSuite) TestSPDKEngineCreateWithSalvageRequested(c *C) {
	...
	engine, err := spdkCli.EngineCreate(engineName, volumeName, types.FrontendSPDKTCPBlockdev, defaultTestLvolSize, replicaAddressMap, nil, 1, true, 0, nil)
	c.Assert(err, IsNil)
	c.Assert(engine, NotNil)
	c.Assert(engine.State, Equals, types.InstanceStateRunning)
...
func (s *TestSuite) spdkMultipleThreadFastRebuilding(c *C, withBackingImage bool) {
	...
	for i := range concurrentCount {
		go func(idx int) {
			...
			engine, err := spdkCli.EngineCreate(vol.engineName, vol.volumeName, types.FrontendSPDKTCPBlockdev, defaultTestLargeLvolSize, vol.replicaAddressMap, nil, 1, false, 0, nil)
			if err != nil {
	...
	for i := range concurrentCount {
		go func() {
			...
			err = spdkCli.EngineDelete(engineName)
			c.Assert(err, IsNil)
			engine, err = spdkCli.EngineCreate(engineName, volumeName, types.FrontendSPDKTCPBlockdev, defaultTestLargeLvolSize, replicaTmpAddressMap, nil, 1, false, 0, nil)
			c.Assert(err, IsNil)
			c.Assert(engine.State, Equals, types.InstanceStateRunning)
			...
			engine, err = spdkCli.EngineCreate(engineName, volumeName, types.FrontendSPDKTCPBlockdev, defaultTestLargeLvolSize, replicaAddressMap, nil, 1, false, 0, nil)
			c.Assert(err, IsNil)
			c.Assert(engine.State, Equals, types.InstanceStateRunning)
			c.Assert(engine.ReplicaAddressMap, DeepEquals, replicaAddressMap)
			c.Assert(engine.ReplicaModeMap, DeepEquals, map[string]types.Mode{replicaName2: types.ModeRW})
...
func revertSnapshot(c *C, spdkCli *client.SPDKClient, snapshotName, volumeName, engineName, engineFrontendName string, replicaAddressMap map[string]string) {
	...
	if prevFrontend != types.FrontendEmpty {
		...
		err = spdkCli.EngineDelete(engineName)
		c.Assert(err, IsNil)
		engine, err = spdkCli.EngineCreate(engineName, volumeName, types.FrontendEmpty, volumeSize, replicaAddressMap, nil, 1, false, 0, nil)
		c.Assert(err, IsNil)
		c.Assert(engine.State, Equals, types.InstanceStateRunning)
		c.Assert(engine.ReplicaAddressMap, DeepEquals, replicaAddressMap)
	...
	if prevFrontend != types.FrontendEmpty {
		...
		err = spdkCli.EngineDelete(engineName)
		c.Assert(err, IsNil)
		engine, err = spdkCli.EngineCreate(engineName, volumeName, prevFrontend, volumeSize, replicaAddressMap, nil, 1, false, 0, nil)
		c.Assert(err, IsNil)
		c.Assert(engine.State, Equals, types.InstanceStateRunning)
		c.Assert(engine.ReplicaAddressMap, DeepEquals, replicaAddressMap)
		c.Assert(engine.Port, Not(Equals), int32(0))
...
func (s *TestSuite) TestSPDKEngineReplicaAddWithoutEngineFrontendInfo(c *C) {
	...
	_, err = spdkCli.EngineCreate(engineName, volumeName, types.FrontendEmpty, defaultTestLvolSize, replicaAddressMap, nil, 1, false, 0, nil)
	c.Assert(err, IsNil)
